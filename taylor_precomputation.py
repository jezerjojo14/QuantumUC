...
#     output_expression=[0 for _ in expression]
#     for comb in itertools.combinations(list(range(len(expression)+power-1)), len(expression)-1):
#         # print("Comb:", comb)
#         b=[comb[0]]+[comb[i+1]-comb[i]-1 for i in range(len(expression)-2)]+[len(expression)+power-2-comb[-1]]
#         # print("b", b)
#         qubits=[False for _ in range(n)]
#         for i in range(len(b)):
#             if b[i]==0:
#                 # print("Skip")
#                 continue
#             i_bin=format(i, 'b').zfill(n)[::-1]
#             # print("i in binary:", i_bin)
#             for j in range(n):
#                 if i_bin[j]=="1":
#                     qubits[j]=True
#         # print("Qubits:", qubits)
#         output_expression[sum([(2**q)*int(qubits[q]) for q in range(n)])]+=np.prod([expression[i]**b[i] for i in range(len(expression))])*np.math.factorial(power)/(np.prod([np.math.factorial(b_el) for b_el in b]))
#     str(bin(i))[2:]
...
# print(power_expansion(3,[0,3,5,0,7,0,0,0], 2))
...
def get_asin_x_inv_expression(n, no_terms, x_scale=1):
    ...
    for i in range(len(x_inv_expansion)):
        ...
        if x_inv_expansion[i]!=0:
            # print(x_inv_expansion)
            # print("x^-1 expression", x_inv_expression)
            # print("x^-1 expansion coeff", x_inv_expansion[i])
...
def construct_exp_k_abs_cos_circuit(n, no_terms, k):
    ...
    if str(n)+"_"+str(no_terms)+"_"+str(k) in data.keys():
        cos_expression=data[str(n)+"_"+str(no_terms)+"_"+str(k)]
    else:
        cos_expression=get_cos_expression(n, no_terms)
